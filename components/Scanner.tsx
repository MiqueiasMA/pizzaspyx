...
const Scanner: React.FC<ScannerProps> = ({ onSaveLead, isLeadSaved, onOpenAnalysis, onOpenPitch }) => {
  ...
  const handleScan = async (isLoadMore = false) => {
    ...
    try {
      ...
      console.error(error);
      setDebugLog(`ERRO: ${error.message || 'Erro desconhecido'}`);
    } finally {
      setLoading(false);
    ...
    <div className="h-full overflow-y-auto p-4 md:p-8 pb-24">
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div className="glass-panel p-6 rounded-2xl border-zinc-800 shadow-xl relative overflow-hidden">
          ...
          {debugLog && (
            <div className="mt-4 p-3 bg-red-900/20 border border-red-500/50 rounded-lg animate-fadeIn">
              <p className="text-[10px] font-mono text-red-400 break-words">
                <i className="fas fa-bug mr-2"></i> {debugLog}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          {loading && (
            <div className="py-20 text-center space-y-4 animate-pulse">
              <i className="fas fa-satellite-dish text-4xl text-[#66CC00]"></i>
              <p className="text-zinc-500 text-sm font-mono uppercase tracking-[0.2em]">Rastreando Satélites...</p>
            </div>
          )}

          {!loading && results.length === 0 && !debugLog && (
            ...
            return (
              <div key={idx} className="glass-panel p-5 rounded-2xl border-zinc-800 hover:border-[#66CC00]/50 transition-all group animate-fadeIn" style={{animationDelay: `${(idx % 10) * 100}ms`}}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => onOpenAnalysis(result)} className="bg-[#18181b] hover:bg-[#27272a] text-zinc-300 py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 border border-zinc-800">
                    <i className="fas fa-microscope text-blue-400"></i> RAIO-X
                  </button>
                  <button onClick={() => onOpenPitch(result)} className="bg-[#18181b] hover:bg-[#27272a] text-zinc-300 py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 border border-zinc-800">
